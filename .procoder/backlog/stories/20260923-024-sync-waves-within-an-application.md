...
- Real cluster: Kind e2e `should run hooks and waves in order for a real rollout` (fixture `staged/`: pre-sync Job, wave-0 Deployment, wave-1 ConfigMap, post-sync Job) proves the order from cluster timestamps; 10/10 specs pass. Its first run found that, without self-heal, an in-progress multi-group rollout was mistaken for drift of a finished one and stalled; fixed, and the envtest wave case now runs with `selfHeal: false` (fails without the fix, mutation checked).
