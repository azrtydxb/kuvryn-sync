...
- [ ] Write `TestConsoleStartsWithoutOIDC`: `NewAuth` with only a session
...
- [ ] Run `go test ./internal/console -run TestConsoleStartsWithoutOIDC` and
      record the failure (compile errors or a 200 SPA page for /auth/start).
- [ ] Implement `OIDCEnabled`, `Validate`, the optional discovery in
...
- [ ] Update `internal/cli/console.go` to call `cfg.Validate()` and pass the
      rest config to `NewAuth`; update existing tests to the new signature.
- [ ] Run `go test ./internal/console ./internal/cli` — expected: PASS.
...
- [ ] Write `TestTokenSessionReadsAsTheToken` against an `httptest` TLS
...
- [ ] Run `go test ./internal/console -run TestTokenSessionReadsAsTheToken`
      and record the failure (undefined `TokenClient`).
- [ ] Implement `tokenConfig`, `TokenClient` and the token mode.
- [ ] Run the test again — expected: PASS; run
...
- [ ] Write `TestTokenSignIn` against envtest: create ServiceAccount
...
- [ ] Write `TestTokenNeverLeaves`: sign in, read `/api/me`,
...
- [ ] Write `TestTokenSessionCookieFitsInABrowser`: a real envtest SA token
      ...
      `error=token`.
- [ ] Write `TestTokenSignInRefusesCrossSiteRequests`: a POST with
      `Sec-Fetch-Site: cross-site` gets 403 and no cookie.
- [ ] Write `TestOIDCCookieWithoutMethodStillWorks`: a cookie sealed from
...
- [ ] Run `go test ./internal/console -run 'TestTokenSignIn|TestTokenNeverLeaves|TestTokenSessionCookie|TestOIDCCookieWithoutMethod'`
      and record the failures.
- [ ] Implement `token.go`, the session changes and the route. The review
...
- [ ] Write `TestTokenSessionFollowsRBAC` (envtest): ServiceAccounts
...
- [ ] Write `TestReaderCacheSeparatesTokens`: two identities with the same
      username and groups and different tokens get two builds; the key never
      contains the token.
- [ ] Run both and record the failures.
- [ ] Implement the key, the dispatch and the 401 cookie clearing.
- [ ] Run `go test ./internal/console` — expected: PASS.
...
- [ ] Extend `web/e2e/login.spec.ts`: the page shows a "Sign in with a
...
- [ ] Run `make test-ui` and record the failure.
- [ ] Implement the form with the vendored Azrty `Input` and `Button`
      components, the error texts for `token` and `cluster`, and the
      console-dev changes.
- [ ] Run `make test-ui` — expected: all Playwright tests pass; run
...
- [ ] Produce the goldens by rendering `git show v0.4.2:charts/kuvryn-sync`
      ...
      objects.
- [ ] Write `TestConsoleChartTokenOnly`: with only `console.enabled=true`
...
- [ ] Run `go test ./internal/controller -run TestConsoleChart` and record
      the failure.
- [ ] Change the template: `$oidc := or issuerURL clientID`; fail unless
      both are set; wrap the ClusterRole, ClusterRoleBinding, OIDC args,
      claim args and redirect requirement in `if $oidc`.
- [ ] Run `go test ./internal/controller` and `helm lint charts/kuvryn-sync` — PASS.
...
- [ ] Write `TestConsoleDocsCoverTokenSignIn`: `docs/console.md` has the
...
- [ ] Rewrite `docs/console.md`: token sign-in first, OIDC optional, the
...
- [ ] Run the test and `make docs-check` — PASS, 0 broken links.
...
- [ ] Rename `## Unreleased` to `## 0.5.0` with an italic one-line summary.
- [ ] Set `version` and `appVersion` to `0.5.0` in `Chart.yaml`.
- [ ] `make build-installer IMG=ghcr.io/azrtydxb/kuvryn-sync:v0.5.0`, then
      `launcher.sh format dist/install.yaml > dist/install.yaml.formatted && mv dist/install.yaml.formatted dist/install.yaml`.
- [ ] Run `make test`, `make lint`, `make test-ui`, `make docs-check`,
...
- [ ] Security self-review of the diff (token leaks, credential
