...
# Migrate a Flux Kustomization to Kuvryn Sync

This moves one Flux `Kustomization` to a Kuvryn Sync `Application` without
deleting or restarting the workloads it manages. Flux applies with
Server-Side Apply as `kustomize-controller`, so Kuvryn Sync must adopt those fields
explicitly.
...
Point Kuvryn Sync at the same Git repository and path. Keep `automatic: false` so
...
The plan lists every field Kuvryn Sync takes from `kustomize-controller`:
...
Wait until the Kuvryn Sync Application is Synced and Healthy:
...
Where Kuvryn Sync applied the same values Flux had, Server-Side Apply records both
