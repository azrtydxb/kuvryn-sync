...
| Application | `Ready`              | `False` with a [failure reason](#failure-reasons) when the Application cannot start a rollout: an invalid spec (`ValidationFailure`), its service account, its source (`SourceFailure`), or blocked retries. Render, apply, and health failures are recorded in `status.failure` and on the Revision instead. |
