...
| `spec.sync.conflictPolicy`                | `fail` (default) stops on SSA ownership conflicts; `adopt` takes over the conflicting fields, listing each field and previous manager in the plan. Fields held only by the legacy `before-first-apply` owner are taken over under both.                                  |
...
| `spec.health.timeout`                     | Health observation timeout, counted from the start of each rollout attempt: a retry, a redeployed Revision and a self-heal repair each get their own.                                                                                                                    |
...
| `status.deployedDesiredStateHash`         | Desired-state hash the last completed, Healthy rollout deployed; `spec.desiredStateHash` follows every render. A manual rollback approves only this desired state. |
...
| Application | `Ready`              | `True` with `Healthy` after the last rollout completed Synced and Healthy. `False` with a [failure reason](#failure-reasons) when the Application cannot start a rollout (an invalid spec, its service account, its source, or blocked retries) or a rollout fails (render, validation, plan, apply, prune or health), and `False` with `RolledBack` while a completed rollback, manual or automatic, holds the desired revision. `False` with `Degraded` or `Progressing` when a finished rollout's resources are no longer Healthy, keeping `RolledBack` while the desired revision is held, and `True` again once they are. It keeps its last value, with no transition, while a rollout is planned, awaits approval or dependencies, or is progressing, while drift is reported without `selfHeal` and health is unchanged, and while `spec.suspend` is set, so a Ready `True` Application may be drifted or suspended. Its `observedGeneration` is the generation it was computed for. |
...
| Application | `RollbackApproved`                          | Normal  | A manual rollback on an Application with manual sync approved its target's plan under the requester.                            |
| Application | `RollbackTargetChanged`                     | Warning | A manual rollback's target is no longer the Revision or desired state the requester chose, so it waits for `ksync sync`.        |
...
| `sync.kuvryn.io/rollback-target-revision`     | Application (annotation)     | CLI                | Revision object the rollback was requested to; the webhook refuses one that does not exist, belongs to another Application or is of another commit.                                                                                      |
| `sync.kuvryn.io/rollback-requested-by`        | Application (annotation)     | webhook            | Who requested the rollback, recorded when its target, chosen Revision or kind changes and not settable by hand while webhooks run; under manual sync, the approver of the chosen Revision. See [Rollback](concepts.md#rollback).         |
| `sync.kuvryn.io/rollback-requested-at`        | Application (annotation)     | webhook            | When the rollback request was recorded.                                                                                                                                                                                                  |
| `sync.kuvryn.io/rollback-target-hash`         | Application (annotation)     | webhook            | Fingerprint of the desired state the chosen Revision's last completed rollout deployed and the Application's `spec.sync` and `spec.strategy` at request time; the rollback approves only while they are unchanged, and never without it. |
