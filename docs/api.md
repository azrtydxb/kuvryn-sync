...
| Application | `RollbackAbandoned`                         | Warning | A rollback's target could not be resolved, rendered, deployed, or retried, so the request was removed.                          |
...
| `solder.io/rollback-from`                | Application (annotation)     | CLI or Solder | Source revision rolled back from, held once the rollback completes. When missing, Solder records the desired revision.                             |
