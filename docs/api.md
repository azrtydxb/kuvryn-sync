...
| `sync.kuvryn.io/rollback-requested-by`        | Application (annotation)     | webhook            | Who requested the rollback, recorded when its target or kind changes and not settable by hand; under manual sync, a manual rollback's approver. See [Rollback](concepts.md#rollback). |
