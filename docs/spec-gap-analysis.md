...
- Public CRDs: `Repository`, `Application`, and `Revision` under `sync.kuvryn.io/v1alpha1`.
...
- Live resource graph and deterministic root-cause diagnosis, recorded in `Application.status.diagnosis` with a `Diagnosed` Event and printed by `ksync diagnose` and `ksync graph`.
