...
func TestSolderConfigCannotLinkOutOfTheRepository(t *testing.T) {
	...
	if err := os.Symlink(filepath.Join("..", "apps", ".solder.yaml"), filepath.Join(workspace, "inner", ".solder.yaml")); err != nil {
		t.Fatal(err)
	}
	apps, found, err = applicationsFromSolderFile(repo, workspace, "inner/.solder.yaml")
	if err != nil || !found || len(apps) != 1 || apps[0].Name != "mine" {
		t.Fatalf("a link that stays inside the repository must still load: %v %v %v", apps, found, err)
	}
	apps, found, err = applicationsFromSolderFile(repo, workspace, "missing/.solder.yaml")
