...
setup_solder() {
  ...
  for i in $(seq 1 60); do printf 'apiVersion: cert-manager.io/v1\nkind: Issuer\nmetadata: {name: probe, namespace: cert-manager}\nspec: {selfSigned: {}}\n' | $K apply --dry-run=server -f - >/dev/null 2>&1 && break; sleep 2; done
...
Y
  ...
  for i in $(seq 1 60); do
    ...
    [ "$phase" = AwaitingApproval ] && break; sleep 3
  done
  ...
  for i in $(seq 1 60); do
    ...
    [ "$state" = Synced/Healthy ] && break; sleep 3
  done
