...
test-e2e-existing-cluster: require-e2e-img manifests generate fmt vet ## Run e2e tests against current kubeconfig with a prebuilt IMG; no local Kind/Docker setup.
	IMG=$(IMG) CERT_MANAGER_INSTALL_SKIP=true go test -tags=e2e ./test/e2e/ -v -ginkgo.v
...
build-installer: manifests generate kustomize ## Generate a consolidated YAML with CRDs and deployment without dirtying config.
	mkdir -p dist
	tmp="$$(mktemp -d)"; cp -R config "$$tmp/config"; cd "$$tmp/config/manager" && "$(abspath $(KUSTOMIZE))" edit set image controller=${IMG}; cd "$(CURDIR)"; "$(KUSTOMIZE)" build "$$tmp/config/default" > dist/install.yaml; rm -rf "$$tmp"

.PHONY: validate-installer
validate-installer: build-installer ## Server-side dry-run the generated raw installer.
